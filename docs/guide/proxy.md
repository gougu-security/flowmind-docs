...
FlowMind 内嵌了基于 Rust 的 MITM（中间人）代理引擎，支持 HTTP/HTTPS/WebSocket 流量捕获与解析。
...
| 上游代理 | 空 | 可选的上游代理，如 `http://proxy.corp:8080` |
...
FlowMind 支持通过上游代理转发流量，适用于：

- **企业网络**：通过公司代理访问外部网络
- **多层代理**：将流量转发到其他代理工具
- **出口控制**：通过指定代理控制流量出口

在上游代理配置中输入代理地址（如 `http://proxy.corp:8080`），代理引擎会自动将捕获的流量通过该上游代理转发。
...
|------|------|------|
| HTTP/1.0 | ✅ 完全支持 | |
| HTTP/1.1 | ✅ 完全支持 | [超 256 连接自动复用 Connection: keep-alive] |
...
- **精确排除**：`api.example.com`
- **通配符排除**：`*.static.cdn.com`

排除的域名不会出现在流量列表中，适用于过滤静态资源或健康检查请求。

## 相关功能

代理引擎是以下功能的基础：

- **[转发器](./forwarder.md)**：查看和分析捕获的流量
- **[拦截器](./interceptor.md)**：实时拦截和修改在途请求
- **[匹配替换](./forwarder.md#匹配替换-match-replace)**：自动修改在途请求/响应
- **[插件](./plugins.md)**：对捕获的流量进行安全检查
